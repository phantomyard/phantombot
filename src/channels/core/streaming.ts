...
export interface NarrationControllerOptions {
  ...
  suppress?: () => boolean;
  /**
   * NOTE(#580): the language gate used to live HERE, on the narration buffer.
   * It moved up to the orchestrator's chunk stream (lib/narrationStreamGate.ts)
   * so ACP, the TUI and `ask` — which have no narration classifier of their
   * own — are covered by the same implementation. By the time narration
   * reaches this controller it has already been gated; do not re-add a copy.
   */
}
