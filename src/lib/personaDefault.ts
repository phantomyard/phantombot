...
/** The host-level boot hooks phantombot can own. Linux: linger. */
export type BootHook = "linger";

/**
 * Record/clear OWNERSHIP of a host-level boot hook in the host-global
 * `[boot_hooks]` table. `owned = true` after phantombot created the hook
 * (enable-linger for a Boot autostart); `undefined` removes the record.
 *
 * The Linux teardown consults this before disabling linger: linger is also
 * a `phantombot init` prerequisite and can be set by an admin, and other
 * systemd --user services on the host may depend on it — only tear down
 * what we created. Mirrors `writeAutostartMode`' doctrine: HOST-ONLY key,
 * one writer, an empty table deletes the key rather than writing `{}`.
 *
 * Mutates `config.bootHooks` to match, so an in-memory caller does not have
 * to reload to render the change it just made.
 */
export async function writeBootHook(
  config: Config,
  hook: BootHook,
  ...
  await updateConfigToml(config.configPath, (toml: TomlObject) => {
    const table =
      toml.boot_hooks && typeof toml.boot_hooks === "object"
        ? { ...(toml.boot_hooks as Record<string, unknown>) }
        : {};
    if (owned === undefined) delete table[hook];
    else table[hook] = owned;
    if (Object.keys(table).length === 0) delete toml.boot_hooks;
    else toml.boot_hooks = table;
  });
  const hooks = { ...(config.bootHooks ?? {}) };
  if (owned === undefined) delete hooks[hook];
  else hooks[hook] = owned;
  config.bootHooks = hooks;
}
