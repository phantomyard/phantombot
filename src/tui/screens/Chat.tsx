...
export function ChatScreen(props: {
  ...
  /**
   * The TURN is owned by the session too (phantombot#604, review of
   * 2d345c4): busy state, elapsed clock and activity label ride the
   * session's `TurnStore`, so a screen that remounts mid-turn re-attaches to
   * the still-running turn instead of forgetting it — and a prompt typed
   * after the remount interrupts that same turn rather than starting a
   * second one behind it.
   */
  const turn = props.session.turn;
  const turnSubscribe = useCallback(
    (onStoreChange: () => void) => turn.subscribe(onStoreChange),
    [turn],
  );
  const turnSnapshot = useCallback(() => turn.getSnapshot(), [turn]);
  const turnState = useSyncExternalStore(turnSubscribe, turnSnapshot);
  const busy = turnState.busy;
  const activity = turnState.activity;
  /**
  ...
  // just asked for must not arrive off screen above you. The turn started on
  // the SESSION now, so the screen reacts to it: a fresh `busySince` (a new
  // timestamp every turn) snaps the viewport to the bottom — including when
  // this screen just remounted onto a turn that never stopped running.
  const busySince = turnState.busySince;
  useEffect(() => {
  ...
  /**
   * Send a prompt. A prompt typed WHILE a turn runs interrupts it — "type to
   * interrupt", the design every other channel follows — and that lifecycle
   * (controller, in-flight promise, generation counter, transcript writes)
   * lives on the SESSION (`turnRunner.ts`), so it survives this screen
   * unmounting on any navigation. The screen only dispatches.
   */
  const submit = props.session.submit;
  ...
  useInput((char, key) => {
    ...
    // same one `/stop` uses, so both are recorded identically. The controller
    // is the session's now — it survives this screen unmounting.
    if (key.ctrl && char === "c") {
      props.session.abortTurn("stop");
      return;
    }
    if (key.ctrl && char === "q") {
      props.onQuit();
