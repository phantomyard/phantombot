...
function recoverySystemPrompt(personaName?: string): string {
  ...
  // The user's message is the only thing that decides the reply language —
  // the same rule the channel layer states on a healthy turn. Recovery has
  // no other content to be confused by, so pointing at it is enough.
  const language = `Reply in the SAME LANGUAGE as the user's message below.`;
  return [
    who,
    "",
