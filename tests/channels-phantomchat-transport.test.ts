...
describe("phantomchat transport subscription wire shape", () => {
  ...
  test("sendPresence publishes a signed kind-30315 status p-tagged to every peer", async () => {
    ...
    await transport.sendPresence([peerA, peerB]);

    expect(published.length).toBe(1);
    const ev = published[0]!;
    // NIP-38 presence kind, signed by us, advertising "online".
    expect(ev.kind).toBe(30315);
    expect(ev.pubkey).toBe(getPublicKey(sk));
    expect(ev.content).toBe("online");
    // d + status tags carry the NIP-38 shape; one p-tag per peer so each peer's
    // existing #p subscription delivers it.
    expect(ev.tags).toEqual([
      ["d", "general"],
      ["status", "online"],
      ["p", peerA],
  ...
  test("sendPresence is a no-op when there are no peers", async () => {
    let publishCalls = 0;
    const fakePool: RelayPool = {
      subscribeMany() {
        return { close() {} };
