...
import jevCmd, { runDecisionModel } from "../src/cli/jev.ts";
...
const SAVED_PERSONA = process.env.PHANTOMBOT_PERSONA;
...
// The RAW function, not a bound copy: restoring a bound copy leaves
// `process.stderr.write` a foreign function for every file that runs after.
...
afterEach(async () => {
  ...
  if (SAVED_PERSONA === undefined) delete process.env.PHANTOMBOT_PERSONA;
  else process.env.PHANTOMBOT_PERSONA = SAVED_PERSONA;
...
describe("the citty wiring — what the two subcommands actually run", () => {
  // The notice lives in `jev.ts`'s defineCommand.run closure; driving
  // runDecisionModel({deprecated:true}) directly proves the flow, not the
  // wiring — dropping the flag from the alias would pass those tests while
  // `phantombot jev` went silent. These go through the default exports.
  const savedExitCode = process.exitCode;
  afterEach(() => {
    process.exitCode = savedExitCode;
  });

  test("the alias's run() passes deprecated:true — the notice is wired", async () => {
    await jevCmd.run!({ args: { persona: "phantom", _: [] } } as never);
    expect(process.exitCode).toBe(2);
    expect(errText).toContain("note: `phantombot jev` is deprecated");
    expect(errText).toContain("no persona 'phantom'");
  });

  test("the canonical command's run() forwards with no notice", async () => {
    await decisionModelCmd.run!({ args: { persona: "phantom", _: [] } } as never);
    expect(process.exitCode).toBe(2);
    expect(errText).toContain("no persona 'phantom'");
    expect(errText).not.toContain("deprecated");
  });
