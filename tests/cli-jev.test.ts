...
describe("an unknown vendor name (statedProvider) round-trips through the write path", () => {
  test("apply writes the stated name; a keep re-run is a no-op, picking a transport is a change", async () => {
    process.env.PHANTOMBOT_JEV_API_KEY = "sk-stored";
    await applyDecisionModelConfig({
      config,
      persona,
    ...
    const reloaded = await loadConfig();
    expect(reloaded.jev!.provider).toBe("openrouter");
